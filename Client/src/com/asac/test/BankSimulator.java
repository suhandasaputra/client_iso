 ...
 */
public class BankSimulator extends Thread {
            ...
            try {
                while (true) {
                    ...
                    if (BankServer.getInstance().getIn() != null) {
                        bRec = false;
                        s.delete(0, s.length());
                        System.out.println("ready for listener message from client (bank simu)");
                        BankServer.getInstance();
//                        intData = BankServer.getInstance().getIn().read();

//                        if (intData > 0) {
//                            while (BankServer.getInstance().getIn().available() > 0) {
//                                if (intData == 2) {
//                                    bRec = true;
//                                }
//                                intData = BankServer.getInstance().getIn().read();
...
//                            sendMessage(message);
                            ...
                            if (isoMsg.getMTI().equals("0200")) {
                                if (isoMsg.getString(3).equals("380000")) {
                                    isoMsg.setMTI("0210");
                                    isoMsg.set(39, "00");
                                    isoMsg.set(48, "1234567890      ADI WIBOWO MATAJARI MMS                 1000000100000000000000000000000MATAJARI CORPORATION                    BAYAR ANGSURAN UANG SPP PEMBANGUNANAN   ");
                                    isoMsg.set(54, "1000360C000100000000");
                                } else if (isoMsg.getString(3).equals("100000")) {
                                    isoMsg.setMTI("0210");
                                    isoMsg.set(39, "00");
...
//                Logger.getLogger(ReceiverMessage.class.getName()).log(Level.SEVERE, null, ex);
                ...
                Logger.getLogger(BankSimulator.class.getName()).log(Level.SEVERE, null, ex);
            } finally {
...
//                BankServer.getInstance().getOut().write(("\02" + msg + "\03").getBytes());
