 ...
 */
public class ABDSimulator_1 extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            try {
                while (true) {
                    message = "";
                    if (ABDServer_1.getInstance().getIn() != null) {
                        bRec = false;
                        s.delete(0, s.length());
                        System.out.println("ready for listener message from client");
                        ABDServer_1.getInstance();
//                        intData = ABDServer_1.getInstance().getIn().read();

//                        if (intData > 0) {
//                            while (ABDServer_1.getInstance().getIn().available() > 0) {
//                                if (intData == 2) {
//                                    bRec = true;
//                                }
//                                intData = ABDServer_1.getInstance().getIn().read();
...
//                        message = grabMessageHeaderLengthHexaByte();
                        ...
                        if (message.equals("connection down")) {
                            ABDServer_1.getInstance().destroy();
                        } else {
                        if (!message.equals("")) {
//                            sendMessage(message);
...
//                if ((TimeFunction.getSecondInDay() - start) < 20) {
            if (ABDServer_1.getInstance().getOut() != null) {
//                ABDServer_1.getInstance().getOut().write(("\02" + msg + "\03").getBytes());

                ABDServer_1.getInstance().getOut().write((digitHeader(msg)).getBytes());
                ABDServer_1.getInstance().getOut().flush();
//                        sendingSuccess = true;
            }
//                } else {
//                    sendingSuccess = true;
